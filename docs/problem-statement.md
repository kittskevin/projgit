...
| Synthesised `.git/` | **A1 variant shipped (2026-05-17) as the default; `--no-dotgit` opts out** | The `RootOverlay` mechanism (Phase 1) carries a minimal `.git/HEAD` + `config` + `objects/info/alternates` pointing at the shared store, so `git rev-parse`, `git log`, `cargo` VCS detection, and IDE repo-root detection all work out of the box. A2 (symbolic `HEAD` per ref) and A3 (writable illusion) remain deferred per [design/dotgit-synthesis.md](design/dotgit-synthesis.md). |
...
| 3 | `cat <path>` returns correct bytes within **one upstream RTT** on cold; under **1 ms** on warm. | **Partially met (cold path is structural).** Warm is met (~0.5 ms per the bench). Cold first-read currently runs ~3× slower than `git cat-file` cold; an investigation into fetch coalescing (see [design/fetch-coalescing.md](design/fetch-coalescing.md) §9.5) found that batching strategies do not close the gap — the dominant cost is server-side per-OID work and local pack writes, which batching does not address. Treated as network-bound by design; no current optimization path. |
| 4 | `git log <path>` works inside the mount and returns real history within **one upstream RTT** for a typical query. | **Met for Ref / Commit projections.** The default A1 `.git/` synthesis (see [design/dotgit-synthesis.md](design/dotgit-synthesis.md)) makes `git rev-parse HEAD`, `git log`, and `git cat-file` work inside the mount; a network-gated end-to-end test in `crates/projgit-fuse/tests/mount_real_remote.rs` exercises the full path. `--subtree` mounts skip synthesis (subtree HEAD semantics are ambiguous) and `--no-dotgit` opts out. |
